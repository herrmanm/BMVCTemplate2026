...
The RGB and depth images transferred via the ROS2 network are fed into multiple AI nodes. For object/cube detection, a simple YOLO[XX] network is used that has been finetuned with data from the black and white cubes used for the task. The output provides information about the position and orientation in the image, and the type of object, respectively, the topside letter. The pose estimation node uses mmpose [XXgenaues Modell erwähnen\url{https://mmpose.readthedocs.io/en/latest/model\_zoo\_papers/algorithms.html}]  to estimate the skeletal posture (COCO 17-Point Keypoint Names [XX]) of people standing in front of the camera.
