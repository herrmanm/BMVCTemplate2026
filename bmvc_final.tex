...
There are three different modes of the digital twin. First, it can act as a digital shadow that only displays the current robot configuration and AI node states. Second, the DT operates as a movement planning tool. When objects are moved, commands are sent to the real robot system to recreate these changes. Finally, the DT can be used for simulation and reinforcement learning purposes without real hardware.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{media/scematic_production_cell.pdf}
...
To create the objects, a helper ROS2 node is used. It subscribes to the estimated bounding boxes of the cube detection and sends a command to \textit{Unity} to spawn a distinct cube. The node also keeps track of already detected cubes, to prevent multiple spawns of the same cube.
...
The start and destination positions are sent to the running \textit{moveIt}~\footnote{\url{https://moveit.ai/}} node, where a trajectory is calculated. Since the gripper is also included in URDF and SRDF files, it is also calculated via \textit{moveIt}, sending commands to a distinct gripper controller. This is done for every displaced cube in the simulation.
...
\subsection{Cube placement}
\label{ex:placement}
This experiment shows the precision of the cube detection for recreating the current workspace.

\subsubsection{Setup}
The table represents the workspace and has holes drilled at $2.5\,\mathrm{cm}$ intervals. These holes serve as a metric to determine placement. In the simulation, the texture of the table has been edited to have markings on the surface, that correspond to the holes in the table. They have a distance of $2.5\,\mathrm{cm}$ on the XZ-plane. Two 3D-printed templates, each with an edge length of $5\,\mathrm{cm}$, are used to fixate the cubes. They are shown in Fig.~\ref{fig:cubeholder}. These templates define the cubes to the surface, and at a height of $5\,\mathrm{cm}$. These templates fit in the holes of the table and are also located in the digital twin environment.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.49\linewidth]{media/cube_templates.png}
    \caption{Cube holder templates}
    \label{fig:cubeholder}
\end{figure}

For the experiment, cubes are now placed inside these templates in the real world. The color and the top side letter is determined by random.
...
\begin{figure}[h]
    ...
    \caption{Both workspaces before the the experiment. \textit{left}: real, \textit{right}: simulation}
    \label{fig:cubeholder}
\end{figure}

\subsubsection{Procedure}
To reproduce the current real workspace, it is scanned with the RGB-D camera mounted on the robot. A trajectory, to move the tool center point orthogonal to the tables surface, is planned. It starts at the right-most side and moves at a constant speed to the left-most side. During the movement, the cube detection node estimates the pixel coordinates of the bounding box. With this information, the cube spawner nodes calculates the world coordinates, by using the intrinsic camera parameter and the depth values. This pose info is then sent to and interpret by \textit{Unity}, where the cubes are spawned with the correct top side letter. 

\subsubsection{Evaluation}
XX: Kameraparamter rechen Pixelkoordinaten in Weltkoordinaten um

\subsection{Cube movement}
The second experiment shows, that it is possible to plan a movement of the digital twin, that is then executed in the real world.
\subsubsection{Setup}
The scanned workspace from the previous experiment can be used as a basis and the end position of the robot will be used as the now start position. Cubes are now moved around the workspace and places anywhere inside. The movement is done by the drag-and-drop function with the PC-mouse. To indicate the new position, a ghost model of the cube is displayed. When the dislocation has been done, this ghost model can be used to show the placement.

\subsubsection{Procedure}
In \textit{Unity}, an arbitrary number of cubes are selected and moved in the XZ-plane. To show the precision of the placement, a pyramid of 4 cubes in hight is stacked.
\subsubsection{Evaluation}
...
XX: Cubedetection: Neues mdoell mit rotation
XX: dynamische Erstellung von anderen Objekten in der Simulation
